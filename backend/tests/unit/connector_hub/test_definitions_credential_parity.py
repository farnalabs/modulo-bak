...
These tests pin the reconciled set and assert per-connector-type parity plus
...
# The connector types reconciled in FAR-515: each has a library integration
# definition whose credential_fields are the contract the hub must read.
RECONCILED_TYPES: list[str] = ["datadog", "sentry", "pagerduty", "n8n", "gitlab", "azure_pipelines"]
...
@pytest.mark.parametrize("connector_type", RECONCILED_TYPES)
...
async def test_connector_configured_via_definitions_is_resolvable(connector_type: str) -> None:
    ...
    ci = _FakeCI(
        ...
        config_json=_CONFIGS[connector_type],
...
def test_connector_builds_from_definition_credentials(connector_type: str) -> None:
    ...
    definition = _definition_for(connector_type)
    creds = dict.fromkeys(definition["credential_fields"].keys(), "test-value")
    connector = _build_connector(connector_type, _CONFIGS[connector_type], creds)
