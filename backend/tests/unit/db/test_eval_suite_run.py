...
def test_single_migration_head() -> None:
    ...
    # 0158_sso_provider_id (FAR-457/FAR-464) chains off 0157 and is the head.
    ...
    # 0159_add_missing_foreign_keys (DB-improvement FK sweep) chains off 0158_sso_provider_id.
    chaining_off_0158 = [p for p in revisions if parents[p] == "0158_sso_provider_id"]
    assert [_basename(p) for p in chaining_off_0158] == ["0159_add_missing_foreign_keys.py"]
    # 0160_add_check_constraints chains off 0159.
    chaining_off_0159 = [p for p in revisions if parents[p] == "0159_add_missing_foreign_keys"]
    assert [_basename(p) for p in chaining_off_0159] == ["0160_add_check_constraints.py"]
    # 0161_promote_uuid_fk_columns chains off 0160.
    chaining_off_0160 = [p for p in revisions if parents[p] == "0160_add_check_constraints"]
    assert [_basename(p) for p in chaining_off_0160] == ["0161_promote_uuid_fk_columns.py"]
    # 0162_add_hot_query_indexes chains off 0161 and is the head.
    chaining_off_0161 = [p for p in revisions if parents[p] == "0161_promote_uuid_fk_columns"]
    assert [_basename(p) for p in chaining_off_0161] == ["0162_add_hot_query_indexes.py"]
    # Nothing chains off 0162 -> it is the single head.
    chaining_off_0162 = [p for p in revisions if parents[p] == "0162_add_hot_query_indexes"]
