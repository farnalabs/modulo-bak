...
def test_single_migration_head() -> None:
    ...
    # -> 0138_json_to_jsonb_standardize (this branch's json->jsonb cleanup).
    ...
    assert [_basename(p) for p in chaining_off_0137] == ["0138_json_to_jsonb_standardize.py"]
    # Nothing chains off 0138 -> it is the single head.
    chaining_off_0138 = [p for p in revisions if parents[p] == "0138_json_to_jsonb_standardize"]
