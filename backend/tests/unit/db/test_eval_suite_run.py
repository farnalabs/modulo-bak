...
def test_single_migration_head() -> None:
    ...
    # -> 0137_eval_suite_run -> 0138_spend_ceiling (FAR-391)
    # -> 0139_extend_runs_status_cost_ceiling (FAR-391).
    ...
    # FAR-391 chains its spend-ceiling migrations off main's head.
    chaining_off_0137 = [p for p in revisions if parents[p] == "0137_eval_suite_run"]
    assert [_basename(p) for p in chaining_off_0137] == ["0138_spend_ceiling.py"]
    chaining_off_0138 = [p for p in revisions if parents[p] == "0138_spend_ceiling"]
    assert [_basename(p) for p in chaining_off_0138] == ["0139_extend_runs_status_cost_ceiling.py"]
    # Nothing chains off 0139 -> it is the single head.
    chaining_off_0139 = [p for p in revisions if parents[p] == "0139_extend_runs_status_cost_ceiling"]
