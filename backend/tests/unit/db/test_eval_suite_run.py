...
def test_single_migration_head() -> None:
    ...
    # 0162_add_missing_foreign_keys (DB-improvement FK sweep) chains off 0161.
    chaining_off_0161 = [p for p in revisions if parents[p] == "0161_accounts_must_change_password"]
    assert [_basename(p) for p in chaining_off_0161] == ["0162_add_missing_foreign_keys.py"]
    # 0163_add_check_constraints chains off 0162.
    chaining_off_0162 = [p for p in revisions if parents[p] == "0162_add_missing_foreign_keys"]
    assert [_basename(p) for p in chaining_off_0162] == ["0163_add_check_constraints.py"]
    # 0164_promote_uuid_fk_columns chains off 0163.
    chaining_off_0163 = [p for p in revisions if parents[p] == "0163_add_check_constraints"]
    assert [_basename(p) for p in chaining_off_0163] == ["0164_promote_uuid_fk_columns.py"]
    # 0165_add_hot_query_indexes chains off 0164 and is the single head.
    chaining_off_0164 = [p for p in revisions if parents[p] == "0164_promote_uuid_fk_columns"]
    assert [_basename(p) for p in chaining_off_0164] == ["0165_add_hot_query_indexes.py"]
    # Nothing chains off 0165 -> it is the single head.
    chaining_off_0165 = [p for p in revisions if parents[p] == "0165_add_hot_query_indexes"]
    assert chaining_off_0165 == []
