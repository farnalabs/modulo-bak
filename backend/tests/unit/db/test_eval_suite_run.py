...
def test_single_migration_head() -> None:
    ...
    # -> 0138_pipeline_edge_ports (FAR-416, renumbered from 0137 to avoid
    #    colliding with eval_suite_run; chains off the eval_suite_run head).
    ...
    # 0138_pipeline_edge_ports chains off 0137_eval_suite_run.
    chaining_off_0137 = [p for p in revisions if parents[p] == "0137_eval_suite_run"]
    assert [_basename(p) for p in chaining_off_0137] == ["0138_pipeline_edge_ports.py"]
    # Nothing chains off 0138 -> it is the single head.
    chaining_off_0138 = [p for p in revisions if parents[p] == "0138_pipeline_edge_ports"]
