...
def test_single_migration_head() -> None:
    ...
    # -> 0138_rest_connector_profile (FAR-412 REST connector profile).
    ...
    assert [_basename(p) for p in chaining_off_0137] == ["0138_rest_connector_profile.py"]
    # Nothing chains off 0138 -> it is the single head.
    chaining_off_0138 = [p for p in revisions if parents[p] == "0138_rest_connector_profile"]
