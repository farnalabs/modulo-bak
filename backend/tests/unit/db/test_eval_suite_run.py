...
def test_single_migration_head() -> None:
    ...
    #    renumbered from 0138 to avoid colliding with eval_versioning; chains off
    #    the pipeline_edge_ports head).
    ...
    # Nothing chains off 0141 except 0139 (this PR's router_no_match migration).
    chaining_off_0141 = [p for p in revisions if parents[p] == "0141_pipeline_edge_ports"]
    assert [_basename(p) for p in chaining_off_0141] == ["0139_add_router_no_match_status.py"]
