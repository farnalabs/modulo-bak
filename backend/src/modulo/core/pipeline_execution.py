    ...
    *,
    ...
    while True:
        ...
        if not node_deadlines:
            if not await _await_first_node(node_started_event, run_done_event, exec_task):
                return
        ...
        if not await _await_progress(node_completed_event, node_started_event, run_done_event, exec_task, remaining):
            return
