...
def _add_node_to_graph(
    ...
    *,
    ...
    if node_type == "sandbox_agent":
        ...
        graph.add_node(node_id, _make_router_pass_fn(node_id))
    elif node_type == "hitl":
        ...
        if node_def.get("agent_id"):
            ...
            graph.add_node(node_id, make_connector_fn(node_def, timeout=timeout))
        else:
            graph.add_node(node_id, make_manual_node_fn(node_def, timeout=timeout))
    else:
        graph.add_node(
            node_id,
            make_node_fn(
