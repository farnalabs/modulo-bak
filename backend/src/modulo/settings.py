...
class Settings(BaseSettings):
    ...
    # Budget of re-dispatch attempts for claimed-but-nodeless SAQ zombies whose
    # retry_policy does not cover "stall" (dispatcher_reconcile nodeless repair,
    # FAR-509). A nodeless zombie executed ZERO nodes, so a re-dispatch cannot
    # double-execute anything; each attempt may still fail to dispatch, so this
    # bounds the re-dispatch loop before the backstop terminal-fail applies.
