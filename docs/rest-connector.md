...
| `verify_tls` | `bool` | `true` | Whether the client verifies the server certificate. Disable only for a self-hosted registry with a self-signed cert. The SSRF guard still blocks loopback and metadata targets. |
